...
- [x] **M3 相册智能同步**：板块内「照片/相册」双底栏（默认照片页）——照片页直连全量接口（不带 albumId 的时间线分页，网页照片视图同款，规避逐相册限流）+ dateTaken 新旧降序、相册页按名称文字排序；点击照片全屏查看原图（左右滑动翻页 + 下载按钮）；长按多选批量顺序下载（通知栏进度）；私密相册密码门；**上传照片**（HAR 逆向四步链路：预上传 → 分片上传 → 完成注册 → 索引刷新，进度 + 通知栏 + 完成提示）；按 userId 持久化元数据缓存；本机两级匹配徽标 ✅ 已交付（84 单元测试全绿）
- [x] **M4 录音同步**：列表（offset/limit 分页）+ 编码文件名还原（类型码：录音机/通话/FM/应用）+ 按需下载（单条局部刷新 + 完成提示 + 通知栏进度）+ 长按多选批量下载 ✅ 已交付
- [x] **M5 通讯录 / 笔记 / 短信**：清单拉取（syncTag 增量）+ JSON 导出；通讯录导入本机系统通讯录（ContactsContract 批量插入）+ 长按多选批量导入/导出；笔记 Markdown 查看/编辑 + **保存到云端**（HAR 逆向 POST /note/note/{id}，标题与正文同步）+ 多选批量导出 md ✅ 已交付
- [x] **M6 上传方向**：相册上传（分片 + serviceToken form 认证）与笔记云端同步均已按用户抓包 HAR 逆向落地 ✅ 已交付（2026-08）
