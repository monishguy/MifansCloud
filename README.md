...
- [x] **M3 相册智能同步**：板块内「照片/相册」双底栏（默认照片页）——照片页合并全部相册按 dateTaken 新旧降序（最新在顶）、相册页按名称文字排序（中文按拼音）；点击照片全屏查看原图（签名直链流式，右下角下载按钮）；长按多选批量顺序下载；私密相册密码门（云端支持未验证，失败诚实报错）；上传入口（媒体选择器→选相册，接口待逆向）；本机两级匹配徽标（本机已有/云端新增/已下载）、按需下载原图到 SAF 备份文件夹 ✅ 已交付（76 单元测试全绿）
