...
- [x] **M3 相册智能同步**：板块内「照片/相册」双底栏（默认照片页）——照片页合并全部相册按 dateTaken 新旧降序（最新在顶）、相册页按名称文字排序（中文按拼音）；点击照片全屏查看原图（签名直链流式，**左右滑动翻页**，右下角下载按钮）；长按多选批量顺序下载（**通知栏进度**）；私密相册密码门；上传入口（接口待逆向）；逐相册容错（503 跳过不阻塞）；**按 userId 持久化元数据缓存**（相册名称/资产元数据，重进秒开）；本机两级匹配徽标、按需下载 ✅ 已交付（76 单元测试全绿）
- [x] **M4 录音同步**：列表（offset/limit 分页）+ 编码文件名还原（类型码：录音机/通话/FM/应用）+ 按需下载（**单条局部刷新 + 完成提示 + 通知栏进度**，不整表刷新）✅ 已交付
- [x] **M5 通讯录 / 笔记 / 短信**：清单拉取（syncTag 增量）+ JSON 导出；通讯录**导入本机系统通讯录**（ContactsContract 批量插入）；笔记标题/正文空值兜底（snippet/extraInfo）+ 导出附 rawJson 原始快照 ✅ 已交付
